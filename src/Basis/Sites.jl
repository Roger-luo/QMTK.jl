...
# use array interface
import Base: eltype, length, ndims, size, indices, eachindex, 
...
indices(x::Sites{L, T, N}) where {L, T, N} = indices(x.data)
indices(x::Sites{L, T, N}, d::Integer) where {L, T, N} = indices(x.data, d)
...
rand!(b::Sites{L, T, N}) where {L <: SiteLabel, T, N} = rand!(Base.GLOBAL_RNG, b)
